<template>
  <div class="strips-wrap">
    <div v-for="a in areas" :key="a.id" class="strip-row">
      <div class="strip-area">
        <span class="strip-icono">{{ a.icono }}</span>
        <div>
          <div class="strip-nombre">{{ a.nombre }}</div>
          <div class="strip-count">{{ trabajosPorArea(a.nombre).length }} trabajos</div>
        </div>
      </div>

      <div class="strip-bar">
        <div
          v-for="t in trabajosPorArea(a.nombre)"
          :key="t.id"
          class="strip-block"
        ...
        <div v-if="!trabajosPorArea(a.nombre).length" class="strip-empty">Sin trabajos</div>
      </div>

      <div class="strip-pct" :style="{ color: pctColor(progProm(a.nombre)) }">
        {{ progProm(a.nombre) }}%
      </div>
    </div>
  </div>
...
function progProm(nombre) {
  const ts = trabajosPorArea(nombre)
...
.strip-area { display: flex; align-items: center; gap: 10px; width: 200px; flex-shrink: 0; }
.strip-icono  { font-size: 18px; flex-shrink: 0; }
